...
class Discogs:
    ...
    def get(request, raw):
        ...
        artist['url'] = reverse('track_discogs-detail', args=[artist['id']], request=request)
    ...
    def list(request, params, raw):
        ...
        return {
            'count': len(artists),
            'prev': build_url('track_discogs-list', params=prev_params, request=request) if prev_params['page_token'] else None,
            'next': build_url('track_discogs-list', params=next_params, request=request) if next_params['page_token'] else None,
            'results': artists,
        }
